...
from worldstereo_wam.utils.logging_config import get_logger
from worldstereo_wam.utils import misc, pytorch_utils
...
class RobotVideoDataset(torch.utils.data.Dataset):
    ...
    def _get(self, idx):
        ...
        # Extract per-camera first-frame images for stereo encoder (before concat)
        stereo_left = None
        stereo_right = None
        if num_cameras >= 2:
            # cam 0 = agentview (external), cam 1 = wrist (egocentric)
            # Use first timestep as the stereo observation pair
            stereo_left = video[0, 0]   # [C, H, W]
            stereo_right = video[1, 0]  # [C, H, W]
        ...
        # Add stereo pair if available (for StereoEncoder conditioning)
        if stereo_left is not None and stereo_right is not None:
            data["stereo_left"] = stereo_left    # [C, H, W], range [-1, 1]
            data["stereo_right"] = stereo_right  # [C, H, W], range [-1, 1]
        return data
